...
def node_recv(conn,rip,rport):
    ...
    if(Type_of_Message == 'Request'):
        ...
        send(Id, msg, mode)
        print("Node1 Sending Reply message to ", Id)
        ...
        Recv_Counter += 1
        Clock += 1
        msg = str(Clock) + " Node1 " + str(Message_ID) + " ACK_Reply"
        send(Id, msg, mode)

    elif(Type_of_Message == 'Release'):
        Clock += 1
        msg = str(Clock) + " Node1 " + str(Message_ID) + " ACK_Release"
        send(Id, msg, mode)
        Request_Queue.get()
...
def recv(): # To receive message in background using thraad
    ...
    sock.bind((ip_addr,node_to_port["Node1"]))
    sock.listen()
...
def cs():
    ...
    print("Requesting CS for Node1")

    Request_Queue.put((Clock, "Node1"))
    driver.Request_Clock_Queue.put((Clock, "Node1"))
    driver.Request_Clock_List.append((Clock, "Node1"))
    
    MSG_ID += 1
    msg = str(Clock) + " Node1 " + str(MSG_ID) + " Request"
    ...
    while(Req_Top[1] != "Node1" or Counter < Total_Nodes - 1):
        time.sleep(1)
        Req_Top = Request_Queue.get()
        Request_Queue.put(Req_Top)
        Counter = Recv_Counter
    ...
    print("executing CS Node1")

    driver.Execution_List.append("Executing critical section of Node1")

    time.sleep(10)

    driver.Execution_List.append("Exiting critical section of Node1")

    print("Finished with CS Node1")
    ...
    print("Sending release msg Node1")
