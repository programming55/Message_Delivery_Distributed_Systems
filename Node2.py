...
def node_recv(conn,rip,rport):
    ...
    if(Type_of_Message == 'Request'):
        ...
        Clock += 1
        msg = str(Clock) + " Node2 " + str(Message_ID) + " Reply" 
        send(Id, msg, mode)
        print("Node2 Sending Reply message to ", Id)
        ...
        Recv_Counter += 1
        Clock += 1
        msg = str(Clock) + " Node2 " + str(Message_ID) + " ACK_Reply"
        send(Id, msg, mode)

    elif(Type_of_Message == 'Release'):
        Clock += 1
        msg = str(Clock) + " Node2 " + str(Message_ID) + " ACK_Release"
        send(Id, msg, mode)
        Request_Queue.get()
        ...
        Clock += 1
        msg = str(Clock) + " Node2 " + str(Message_ID) + " ACK_Other"
        send(Id, msg, mode)
        driver.counter += 1
...
def cs():
    ...
    print("Requesting CS for Node2")

    Request_Queue.put((Clock, "Node2"))
    driver.Request_Clock_Queue.put((Clock, "Node2"))
    driver.Request_Clock_List.append((Clock, "Node2"))
    
    MSG_ID += 1
    msg = str(Clock) + " Node2 " + str(MSG_ID) + " Request"
    ...
    print("executing CS Node2")
    
    driver.Execution_List.append("Executing critical section of Node2")

    time.sleep(10)

    driver.Execution_List.append("Exiting critical section of Node2")
    print("Finished with CS Node2")
    ...
    Clock += 1
    MSG_ID += 1
    print("Sending release msg Node2")

    msg = str(Clock) + " Node2 " + str(MSG_ID) + " Release"
